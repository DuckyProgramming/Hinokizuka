        ...
        switch(this.tab){
            case 11:
                if(dist(this.drag.start.x,this.drag.start.y,this.drag.end.x,this.drag.end.y)>10){
                    entities.walls[types.wall[this.edit.add.wall.type].slice].push(new wall(this.layer,round((this.drag.start.x/2+this.drag.end.x/2-types.wall[this.edit.add.wall.type].interval.x[1])/types.wall[this.edit.add.wall.type].interval.x[0])*types.wall[this.edit.add.wall.type].interval.x[0]+types.wall[this.edit.add.wall.type].interval.x[1],round((this.drag.start.y/2+this.drag.end.y/2-types.wall[this.edit.add.wall.type].interval.y[1])/types.wall[this.edit.add.wall.type].interval.y[0])*types.wall[this.edit.add.wall.type].interval.y[0]+types.wall[this.edit.add.wall.type].interval.y[1],ceil(abs(this.drag.start.x-this.drag.end.x)/20)*20,ceil(abs(this.drag.start.y-this.drag.end.y)/20)*20,this.edit.add.wall.type,entities.walls.length,0,game.zone))
                    for(let a=0,la=entities.walls.length;a<la;a++){
                        for(let b=0,lb=entities.walls[a].length;b<lb;b++){
                            entities.walls[a][b].checkRedundant()
                        }
